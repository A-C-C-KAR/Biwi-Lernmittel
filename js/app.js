        const app = {
            ...
            toggleQuestion(id) {
                ...
                const btnEl = document.getElementById(`accordion-btn-${id}`);
                answerEl.classList.toggle('hidden');
                const isHidden = answerEl.classList.contains('hidden');
                chevron.style.transform = isHidden ? 'rotate(0deg)' : 'rotate(180deg)';
                if (btnEl) btnEl.setAttribute('aria-expanded', !isHidden);
            },
            ...
            expandAll() {
                ...
                document.querySelectorAll('[id^="accordion-btn-"]').forEach(el => el.setAttribute('aria-expanded', 'true'));
            },
            ...
            collapseAll() {
                ...
                document.querySelectorAll('[id^="accordion-btn-"]').forEach(el => el.setAttribute('aria-expanded', 'false'));
            },
                        ...
                        <div class="divide-y divide-stone-100">
                            ${cat.questions.map(q => `
                                <div class="p-6 hover:bg-stone-50/50 transition-colors group">
                                    <button type="button" id="accordion-btn-${q.id}" aria-expanded="false" aria-controls="answer-${q.id}" class="w-full text-left flex items-start justify-between cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 rounded" onclick="app.toggleQuestion(${q.id})">
