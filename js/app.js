                        ...
                        <div class="divide-y divide-stone-100">
                            ${cat.questions.map(q => `
                                <div class="p-6 hover:bg-stone-50/50 transition-colors group">
                                    <button id="btn-question-${q.id}" class="w-full text-left flex items-start justify-between cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 rounded p-1 -m-1" aria-expanded="false" aria-controls="answer-${q.id}" onclick="app.toggleQuestion(${q.id})">
                                        <div class="flex items-start space-x-4">
                                            <span class="font-mono text-stone-400 text-sm mt-1">#${q.id}</span>
                                            <p class="font-medium text-stone-800 text-lg pr-4">${q.q}</p>
                                        </div>
                                        <svg id="chevron-${q.id}" aria-hidden="true" class="w-5 h-5 text-stone-400 transform transition-transform duration-200 mt-1 flex-shrink-0 group-hover:text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                    </button>
